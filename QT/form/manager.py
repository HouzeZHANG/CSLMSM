...
from PyQt5.QtWidgets import QMainWindow, QApplication, QPushButton, QWidget, QAction, QTabWidget, QVBoxLayout, QDialog, \
    QLabel, QComboBox, QLineEdit, QSplitter, QTreeView, QMessageBox, QCheckBox, QAbstractItemView, QHBoxLayout, \
...
class Manager(QWidget):
    ...
    def validate_user(self):
        ...
        if (
                orga != '' and uname != '' and fname != '' and lname != '' and tel != '' and email != '' and password != ''):
    ...
    def validate_right(self):
        ...
        if 'admin' not in [i[0] for i in self.lst] and 'manager' not in [i[0] for i in self.lst]:
            ...
            msg.exec_()
            return
        if (~test_mean_type.isspace() and test_mean_type != '') and (
                ~test_mean_name.isspace() and test_mean_name != '') and (
                ~test_mean_num.isspace() and test_mean_num != ''):
            if '_'.join([test_mean_type, test_mean_name, test_mean_num]) not in [i[2] for i in self.lst if
                                                                                 i[0] == 'admin' and i[
                                                                                     1] == 'test mean'] and 'manager' not in [
                i[0] for i in self.lst]:
                msg = QMessageBox()
                msg.setIcon(QMessageBox.Warning)
                msg.setText("You are not allowed to do this")
                msg.setWindowTitle("Validate Fail")
        ...
        if (~type_coating.isspace() and type_coating != ''):
            if type_coating not in [i[2] for i in self.lst if
                                    i[0] == 'admin' and i[1] == 'coating'] and 'manager' not in [i[0] for i in
                                                                                                 self.lst]:
                msg = QMessageBox()
                msg.setIcon(QMessageBox.Warning)
                msg.setText("You are not allowed to do this")
        ...
        if (~type_detergent.isspace() and type_detergent != ''):
            if type_detergent not in [i[2] for i in self.lst if
                                      i[0] == 'admin' and i[1] == 'detergent'] and 'manager' not in [i[0] for i in
                                                                                                     self.lst]:
                msg = QMessageBox()
                msg.setIcon(QMessageBox.Warning)
                msg.setText("You are not allowed to do this")
        ...
        if (~type_tank.isspace() and type_tank != ''):
            if type_tank not in [i[2] for i in self.lst if i[0] == 'admin' and i[1] == 'tank'] and 'manager' not in [
                i[0] for i in self.lst]:
                msg = QMessageBox()
                msg.setIcon(QMessageBox.Warning)
                msg.setText("You are not allowed to do this")
        ...
        if (~type_sensor.isspace() and type_sensor != ''):
            if type_sensor not in [i[2] for i in self.lst if
                                   i[0] == 'admin' and i[1] == 'sensor'] and 'manager' not in [i[0] for i in self.lst]:
                msg = QMessageBox()
                msg.setIcon(QMessageBox.Warning)
                msg.setText("You are not allowed to do this")
                msg.setWindowTitle("Validate Fail")
        ...
        if (~type_ejector.isspace() and type_ejector != ''):
            if type_ejector not in [i[2] for i in self.lst if
                                    i[0] == 'admin' and i[1] == 'ejector'] and 'manager' not in [i[0] for i in
                                                                                                 self.lst]:
                msg = QMessageBox()
                msg.setIcon(QMessageBox.Warning)
                msg.setText("You are not allowed to do this")
        ...
        if (~type_camera.isspace() and type_camera != ''):
            if type_camera not in [i[2] for i in self.lst if
                                   i[0] == 'admin' and i[1] == 'camera'] and 'manager' not in [i[0] for i in self.lst]:
                msg = QMessageBox()
                msg.setIcon(QMessageBox.Warning)
                msg.setText("You are not allowed to do this")
                msg.setWindowTitle("Validate Fail")
        ...
        if (~type_test_point.isspace() and type_test_point != ''):
            if type_test_point not in [i[2] for i in self.lst if
                                       i[0] == 'admin' and i[1] == 'test point'] and 'manager' not in [i[0] for i in
                                                                                                       self.lst]:
                msg = QMessageBox()
                msg.setIcon(QMessageBox.Warning)
                msg.setText("You are not allowed to do this")
        ...
        if (~type_intrinsic_value.isspace() and type_intrinsic_value != ''):
            if type_intrinsic_value not in [i[2] for i in self.lst if
                                            i[0] == 'admin' and i[1] == 'intrinsic value'] and 'manager' not in [i[0]
                                                                                                                 for i
                                                                                                                 in
                                                                                                                 self.lst]:
                msg = QMessageBox()
        ...
        if (self.tabs.currentIndex() == 1):
            ...
            if obj == self.coating_type:
                ...
                if event.type() == QEvent.KeyPress and (
                    ...
                    if ~type_coating.isspace() and type_coating != '':
                        ...
                        self.test_point_type.setCurrentText('')
                        self.intrinsic_value_type.setCurrentText('')
                        if type_coating not in [self.coating_type.itemText(i) for i in
                                                range(self.coating_type.count())]:
            ...
            if obj == self.detergent_type:
                ...
                if event.type() == QEvent.KeyPress and (
                    ...
                    self.intrinsic_value_type.setCurrentText('')
                    if ~type_detergent.isspace() and type_detergent != '':
                        if type_detergent not in [self.detergent_type.itemText(i) for i in
                                                  range(self.detergent_type.count())]:
            ...
            if obj == self.ejector_type:
                ...
                if event.type() == QEvent.KeyPress and (
                    ...
                    self.intrinsic_value_type.setCurrentText('')
                    if ~type_ejector.isspace() and type_ejector != '':
                        if type_ejector not in [self.ejector_type.itemText(i) for i in
                                                range(self.ejector_type.count())]:
            ...
            if obj == self.test_point_type:
                ...
                if event.type() == QEvent.KeyPress and (
                    ...
                    self.intrinsic_value_type.setCurrentText('')
                    if ~type_test_point.isspace() and type_test_point != '':
                        if type_test_point not in [self.test_point_type.itemText(i) for i in
                                                   range(self.test_point_type.count())]:
            ...
            if obj == self.intrinsic_value_type:
                ...
                if event.type() == QEvent.KeyPress and (
                    ...
                    self.test_point_type.setCurrentText('')
                    if ~type_intrinsic_value.isspace() and type_intrinsic_value != '':
                        if type_intrinsic_value not in [self.intrinsic_value_type.itemText(i) for i in
                                                        range(self.intrinsic_value_type.count())]:
                ...
                if (~test_mean_type.isspace() and test_mean_type != '') and (
                    ...
                    for i in range(self.model_author.rowCount()):
                        ...
                        if (len(d) == 1):
                            ...
                            self.cur.execute(sql, (
                            uname, right, test_mean_type, test_mean_name, test_mean_num, insect, acquisition))
                            self.conn.commit()
                    for i in range(self.model_other.rowCount()):
