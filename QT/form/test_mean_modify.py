...
class testMean(QWidget):
    ...
    def searchParam(self):
        ...
        if not test_mean_type.isspace() and test_mean_type != '' and not test_mean_name.isspace() and test_mean_name != '' and not test_mean_num.isspace() and test_mean_num != '':
            ...
            fname = QFileDialog.getOpenFileName(self, 'Open file',
                                                f'.\\file\\test_mean\\{file_name}_param',
    ...
    def delAttribute(self):
        ...
        test_mean_name = self.test_mean_name.currentText()
        test_mean_num = self.test_mean_num.currentText()
        if (test_mean_type.isspace() or test_mean_type == '') or (test_mean_name.isspace() or test_mean_name == '') or (
            ...
            if event.type() == QEvent.KeyPress and event.key() == Qt.Key_Tab:
                ...
                if not gen.isspace() and gen != '' and not nom.isspace() and nom != '':
                    ...
                    if 'manager' in [i[0] for i in self.lst]:
                        ...
                        for i in self.lst:
                            if i[1].split('_')[0] == gen and i[1].split('_')[1] == nom and i[1].split('_')[
                                2] not in test_mean_num:
        ...
        if obj == self.att_value:
            if event.type() == QEvent.KeyPress and (event.key() == Qt.Key_Return or event.key() == Qt.Key_Enter):
                if self.list_attribute_tree.selectedIndexes()[0]:
                    ...
                    self.model_attribute.setData(self.model_attribute.index(select_row, 0),
                                                 self.attribute.currentText())
                    self.model_attribute.setData(self.model_attribute.index(select_row, 1),
                                                 self.att_unity.currentText())
    ...
    def transfer(self):
        ...
        test_mean_name = self.test_mean_name.currentText()
        test_mean_num = self.test_mean_num.currentText()
        if 'manager' in [i[0] for i in self.lst] or 'admin' in [i[0] for i in self.lst if i[1] == '_'.join(
                [test_mean_type, test_mean_name, test_mean_num])] or 'validate' in [i[0] for i in self.lst if
                                                                                    i[1] == '_'.join([test_mean_type,
                                                                                                      test_mean_name,
                                                                                                      test_mean_num])]:
